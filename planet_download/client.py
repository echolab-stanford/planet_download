...
Source: https://github.com/planetlabs/notebooks/blob/master/jupyter-notebooks/Basemaps-API/basemaps_client.py
"""
...
class BasemapsClient(object):
    ...
    ----------
    base_url : str
        The base URL for the Planet Basemaps API.
    api_key : str
        The Planet API key.
    session : requests.Session
        The requests session with authentication set.
    """
    ...
    def _url(self, endpoint):
        ...
        ----------
        endpoint : str
            The API endpoint to append to the base URL.
    ...
    def _consume_pages(self, endpoint, key, **params):
        ...
        key : str
            The key in the response JSON to yield items from.
        **params
    ...
    def _query(self, endpoint, key, json_query):
        """
        Post and then get for pagination. Used for paginated POST endpoints.
    ...
    def _list(self, endpoint, key=None, **params):
        """
        List items from a paginated endpoint.
        ...
        endpoint : str
            The API endpoint to query.
        key : str, optional
            The key in the response JSON to yield items from. Defaults to the
            endpoint name.
        **params
            Additional parameters for the request.
    ...
    def _item(self, endpoint, **params):
        ...
        **params
        ...
        -------
        dict
            The JSON response for the item.
        """
    ...
    def _download(self, url, filename=None, output_dir=None):
        ...
        filename : str, optional
            The filename to save as. If not specified, will use the filename
            from the Content-Disposition header.
        output_dir : str, optional
            The directory to save the file in. If not specified, saves in the
            current directory.
    ...
    def series(self, name=None, series_id=None):
        """
        Retrieve a MosaicSeries object for a specific series by either name or ID.
    ...
    def list_series(self, name_contains=None):
        """
        Iterate through all mosaic series you have access to, optionally filtering
        based on name.

        Parameters
        ----------
        name_contains : str, optional
            Search only for series that contain the specified substring in their names.
    ...
    def list_mosaics(self, name_contains=None):
        """
        Iterate through all mosaics you have access to, optionally filtering based
        on name.

        Parameters
        ----------
        name_contains : str, optional
            Search only for mosaics that contain the specified substring in their names.
    ...
    def mosaic(self, name=None, mosaic_id=None):
        """
        Retrieve a Mosaic object for a particular mosaic, either by name or ID.
...
class MosaicSeries(object):
    ...
    def from_name(cls, name, client=None):
        ...
        -------
        MosaicSeries
            An instance of MosaicSeries for the given name.
        """
    ...
    def from_id(cls, series_id, client=None):
        ...
        -------
        MosaicSeries
            An instance of MosaicSeries for the given ID.
        """
        client = _get_client(client)
        info = client._item(f"series/{series_id}")
    ...
    def mosaics(self, start_date=None, end_date=None):
        ...
        start_date : str or datetime, optional
            The earliest date to use. This check is based on the first_acquired
            metadata for the mosaic.
        end_date : str or datetime, optional
            The latest date to use. This check is based on the last_acquired
            metadata for the mosaic.

        Yields
        ------
        Mosaic
            An object representing a mosaic in the series.
        """
        params = {}
    ...
    def all_quads_overlap(self, start_date, end_date, bbox=None, region=None):
        """Get all the quads within a time and space range
        ...
        end_date : str or datetime
            ...
            metadata for the mosaic.
        bbox : tuple
            A 4-item tuple of floats. Expected to be (longitude_min, latitude_min,
        ...
        ------
        MosaicQuad
            An object representing a quad in the mosaic.
        """
    ...
    def download_selection(
        self, list_quads, flat=False, filename_template=None, save_dir=None, nthreads=16
    ):
        """Download a selection of quads for all mosaics in the series.

        This is a way to run `download_quads` on a selection of quads only rather
        than on the whole set of quads. We use `all_quads_overlap` to get the
        relevant quads and process the downloading in chunks.
        """

        if flat and not filename_template:
            filename_template = "{mosaic}-L{level}-{x:04d}E-{y:04d}N.tif"

        groups = _chunks(list_quads, 4 * nthreads)

        def download(quad):
            filename, output_dir = None, None
            if not flat:
                output_dir = os.path.join(save_dir, quad.mosaic_name)

            if filename_template is not None:
                filename = filename_template.format(
                    mosaic=quad.mosaic_name, level=quad.level, x=quad.x, y=quad.y
                )
    ...
    def download_quads(
        ...
        start_date : datetime, optional
            The earliest date to use. Based on first_acquired metadata.
        end_date : datetime, optional
            The latest date to use. Based on last_acquired metadata.
        nthreads : int, optional
            Number of concurrent downloads.
        flat : bool, optional
            If True, all quads are placed in the specified folder with the mosaic
            name included in the filename.
        filename_template : str, optional
            A format string with the keys "mosaic", "level", "x", "y". Defaults to
            the Content-Disposition specified by the API.

        Yields
        ------
        str
            The path to each downloaded file.
        """
        if flat and not filename_template:
            filename_template = "{mosaic}-L{level}-{x:04d}E-{y:04d}N.tif"
...
class Mosaic(object):
    ...
    ----------
    info : dict
        The JSON API response for the mosaic from the API.
    client : BasemapsClient, optional
        A specific Client instance to use. Will be created if not specified.
    """
    ...
    def from_name(cls, name, client=None):
        ...
        -------
        Mosaic
            An instance of Mosaic for the given name.
        """
        client = _get_client(client)
    ...
    def from_id(cls, mosaic_id, client=None):
        ...
        -------
        Mosaic
            An instance of Mosaic for the given ID.
        """
        client = _get_client(client)
    ...
    def download_quads(
        ...
        def download(quad):
            if filename_template is not None:
                filename = filename_template.format(
                    mosaic=self.name, level=quad.level, x=quad.x, y=quad.y
                )
            else:
                filename = None
...
class MosaicQuad(object):
    ...
    ----------
    info : dict
        The JSON API response for the quad from the API.
    mosaic : Mosaic, optional
        The parent Mosaic object.
    client : BasemapsClient, optional
        A specific Client instance to use. Will be created if not specified.
    """
    ...
    def __init__(self, info, mosaic=None, client=None):
        """
        Initialize a MosaicQuad instance.
    ...
    def download_url(self):
        ...
        -------
        str or None
            The download URL if available, else None.
        """
    ...
    def download(self, filename=None, output_dir=None):
        ...
        -------
        str or None
            The path to the downloaded file, or None if not downloadable.
        """
    ...
    def contribution(self):
        ...
        -------
        list of str
            List of URLs for each contributing scene.
        """
