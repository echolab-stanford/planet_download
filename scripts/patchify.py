...
Designed for an UNSUPERVISED training pipeline (no train/val/test split): every
valid patch is emitted as a .png. Output layout:
...
Folder sizing:
- Folders are filled to EXACTLY --shard-size patches (default 512) and then a new
  folder is started, so every folder has a constant file count. Patches are
  ordered by (block, year-quarter, quad) first, so each folder stays
  geographically coherent; only the last folder of each worker is partial
  (=> ~<workers> non-full folders out of thousands).
- Geography is not encoded in the folder name (it would be ambiguous at the few
  cross-block boundaries); it lives in manifest.csv and in each PNG's filename
  (<BLOCK>_<YYYYqQ>_<x>_<y>_<rowcol>).
...
- Each manifest row lists any DHS cluster whose point falls inside the patch
  (join back to examples/dhs_sa_africa_cluster.csv on DHSID_EA for pca/urban).

Block + cluster lookups come from crosswalk_quads_planet_*.geojson, so there is
no extra metadata-scan pass.
...
"""
import argparse, csv, json, math, glob, os
...
PATCH = 512
MANIFEST_COLS = ["path", "block", "lat", "lon", "year", "quarter",
                 "quad_x", "quad_y", "row", "col", "mosaic", "valid_frac",
                 "dhsids", "n_clusters"]
...
def read_crosswalks(src, grid_deg):
    ...
    for f in sorted(glob.glob(f"{src}/crosswalk_quads_planet_*.geojson")):
        ...
        for mosaic, qid, dhsid, clat, clon, miny, maxy, minx, maxx in zip(
                g["mosaic_name"], g["id"], g["DHSID_EA"], g["lat"], g["lon"],
                bounds.miny, bounds.maxy, bounds.minx, bounds.maxx):
            key = (mosaic, qid)
            if key not in block_idx:
                block_idx[key] = block_label((miny + maxy) / 2, (minx + maxx) / 2, grid_deg)
...
def patches_for(path, min_valid, clusters):
    ...
    if clusters:
        cx, cy = warp_transform("EPSG:4326", crs,
                                [c[2] for c in clusters], [c[1] for c in clusters])
...
class ChunkWriter:
    ...
    def write(self, key, rgb):
        if self.cur is None or self.in_dir >= self.shard_size:
            self._roll()
        Image.fromarray(rgb).save(self.cur / f"{key}.png", "PNG", compress_level=self.compress)
        self.in_dir += 1; self.total += 1
        return f"{self.cur.name}/{key}.png"


def process_chunk(wid, items, out, shard_size, min_valid, compress, grid_deg):
    """Process a worker's quads (pre-sorted) into uniform shard folders."""
    w = ChunkWriter(out, wid, shard_size, compress)
    rows, errors = [], []
    for path, block, yq, clusters in items:
        try:
            year, quarter, qx, qy = parse_name(path)
            for iy, ix, rgb, lat, lon, frac, dhsids in patches_for(path, min_valid, clusters):
                key = f"{block}_{yq}_{qx}_{qy}_{iy:02d}{ix:02d}"
                rel = w.write(key, rgb)
                rows.append((rel, block_label(lat, lon, grid_deg),
                             round(lat, 6), round(lon, 6), year, quarter, qx, qy,
                             iy, ix, Path(path).parent.name, round(frac, 4),
                             ";".join(dhsids), len(dhsids)))
        except Exception as e:
            errors.append((str(path), str(e)[:160]))
    return {"patches": w.total, "folders": w.idx + 1, "rows": rows, "errors": errors}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--src", required=True, help="dataset dir (global_quarterly_*/ + crosswalks)")
    ap.add_argument("--out", required=True, help="output dir (local/SCRATCH)")
    ap.add_argument("--workers", type=int, default=48)
    ap.add_argument("--grid-deg", type=float, default=1.0, help="lat/lon block size (deg)")
    ap.add_argument("--shard-size", type=int, default=512, help="PNGs per shard folder")
    ap.add_argument("--min-valid", type=float, default=0.5, help="min non-alpha-zero fraction")
    ...
    tifs = [p for d in sorted(glob.glob(args.src + "/global_quarterly_*"))
            for p in glob.glob(d + "/*.tif")]
    print(f"{len(tifs)} quads; resolving blocks and ordering for locality...", flush=True)

    items, unindexed = [], 0
    for p in tifs:
        mosaic, qid = os.path.basename(os.path.dirname(p)), Path(p).stem.replace("_", "-")
        block = block_idx.get((mosaic, qid))
        if block is None:                          # not in crosswalk -> read header
            try:
                with rasterio.open(p) as s:
                    b = s.bounds
                    lons, lats = warp_transform(s.crs, "EPSG:4326",
                                                [(b.left + b.right) / 2],
                                                [(b.bottom + b.top) / 2])
                block = block_label(lats[0], lons[0], args.grid_deg); unindexed += 1
            except Exception:
                continue
        tok = mosaic.split("_")[-2]
        items.append((p, block, tok[:4] + tok[4:], clust_idx.get((mosaic, qid), [])))

    # order by (block, year-quarter, quad) so each shard folder is locally coherent
    items.sort(key=lambda it: (it[1], it[2], it[0]))
    # split into <workers> contiguous chunks (preserves ordering within a chunk)
    W = max(1, args.workers)
    n = len(items)
    chunks = [items[k * n // W:(k + 1) * n // W] for k in range(W)]
    chunks = [c for c in chunks if c]
    print(f"{len(items)} quads ({unindexed} header fallback) -> {len(chunks)} worker chunks", flush=True)

    total_patches, total_folders, all_errors, done = 0, 0, [], 0
    with open(out / "manifest.csv", "w", newline="") as fh:
        wr = csv.writer(fh); wr.writerow(MANIFEST_COLS)
        with ProcessPoolExecutor(max_workers=W) as ex:
            futs = [ex.submit(process_chunk, wid, c, str(out),
                              args.shard_size, args.min_valid, args.compress, args.grid_deg)
                    for wid, c in enumerate(chunks)]
            for f in as_completed(futs):
                r = f.result(); done += 1
                wr.writerows(r["rows"])
                total_patches += r["patches"]; total_folders += r["folders"]
                all_errors += r["errors"]
                print(f"  chunk {done}/{len(futs)} done; {total_patches} patches, "
                      f"{total_folders} folders so far", flush=True)

    (out / "patchify_summary.json").write_text(json.dumps(
        {"patches": total_patches, "folders": total_folders, "shard_size": args.shard_size,
         "avg_per_folder": round(total_patches / max(total_folders, 1), 1),
         "grid_deg": args.grid_deg, "min_valid": args.min_valid,
         "n_errors": len(all_errors), "errors": all_errors[:200]}, indent=2))
    print(f"DONE: {total_patches} patches in {total_folders} folders "
          f"(~{total_patches / max(total_folders,1):.0f}/folder); {len(all_errors)} read errors.",
          flush=True)
